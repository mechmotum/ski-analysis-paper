...
\begin{abstract}
  Terrain parks are now present in most snowsports resorts and for decades the
  ...
  been done to make jumps safer by design due to an apathetic ski industry and
  ...
  danger. With the tools available, the barriers to designing safer jumps is
  further lowered for the ski industry to address the epidemic.
\end{abstract}
...
around the world from skydiving~\todo{add citation} to children's playground
...
surface profile along the jumper's path and takeoff angle are
available~\cite{Hubbard2012}. Reduction the equivalent fall height is one of
the dominant jump design factors that can reduce injury. Equivalent fall height
is can be controlled for and it is an obvious safety design factor to minimize
without loss of enjoyment from ski and snowboard participants.

This paper reiterates the societal cost of the increasing terrain park injuries and
then focuses on specific case studies that highlight the danger incurred if
...
workplace safety~\todo{need citation} and the FAA's safety regulations for safe
parachuting landings is a 3~\si{\meter} fall height~\todo{need citation}.
...
call for equivalent fall heights of 0.46~\si{\meter}~\todo{need citation}.
...
Park Profiler~\todo{need citation}. Differential GPS has become less expensive
