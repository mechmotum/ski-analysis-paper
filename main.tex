...
% additional packages
% for \url{} command
% commented out because the sage template uses this
\usepackage[hidelinks]{hyperref}
% useful for mathematical formulas and symbols
...
%\runninghead{Moore~et~al.}

\title{Preprint Version 7\\Safety-Conscious Design of Terrain Park Jumps: Ethical
Issues and Online Software}

%\author{
  %Jason~K. Moore\affilnum{1} and
  %Bryn Cloud\affilnum{2} and
  %Mont Hubbard\affilnum{2} and
  %Christopher~A. Brown\affilnum{3}
%}
%
%\affiliation{
  %\affilnum{1}Delft University of Technology\\Mekelweg 2, 2628 CD Delft, The
  %Netherlands\\
  %\affilnum{2}University of California, Davis\\One Shields Ave., Davis, CA
  %95616 USA\\
  %\affilnum{3}Worcester Polytechnic Institute\\100 Institute Rd., Worcester, MA
  %01609 USA
%}
%
%\corrauth{Jason~K. Moore,
...
%\keywords{skiing, snowboarding, safety, ethics}
...
Levy~et~al.~\cite{Levy2015}. These coordinates include starting points, takeoff ramps, and
...
research~\cite{Smith2020} tells us that designing forgiving environments
...
most skiing and snowboarding deaths~\cite{CPSC1999}. Koehle~et~al.~\cite{Koehle2002} stated
``[S]eventy-seven percent of spinal injuries~\cite{Tarazi1999} and 30\% of
head injuries~\cite{Fukuda2001} in snowboarding were a result of jumps.''
Jackson~et~al.~\cite{Jackson2004} determined that by 2004 snow skiing replaced football as the
...
Russell~et~al.~\cite{Russell2014}, ``between 5 and 27\% of skiing and snowboarding injuries
...
Audet~et~al.~\cite{Audet2020} provides evidence that skiing or snowboarding in a terrain
park is a risk factor for head, neck, back, and other severe injuries.
Hosaka~et~al.~\cite{Hosaka2020} concludes that jumping is a main cause for serious spinal
...
respectively~\cite{Vish2005}. The German Society for Trauma Surgery's
...
general workplace safety~\cite{OSHA2021}. Chalmers~et~al.~\cite{Chalmers1996} argues for
...
1.5~\si{\meter} for terrain park jumps requiring basic skills~\cite{Heer2019}.
...
orientations~\cite{Scher2015}, although ramp curvature does not influence
EFHs.
...
specified EFH in Moore~et~al.~\cite{Moore2018}. It comprises a general-purpose, extensible,
...
\begin{figure*}
  \centering
  %\includegraphics[width=6in]{figures/web-app-screenshot.png}
...
SciPy~\cite{Virtanen2020}, SymPy~\cite{Meurer2017}, and xlrd. This software
is open source and licensed under the MIT redistribution license. The source
code is distributed on
PyPi~\footnote{\url{https://pypi.org/project/skijumpdesign}}.  Users can submit
bug reports, feature requests, code improvements, and additions at the Gitlab
...
Simulations below use methods in Levy~et~al.~\cite{Levy2015}, assuming the same skier mass,
...
\begin{figure*}
  \centering
  % the figure width is the column width of the original Springer LaTeX
  % template
  \includegraphics[width=4.67690604676906in]{figures/vine-v-bear-valley.pdf}
...
design, see Levy~et~al.~\cite{Levy2015}, of similar size with similar flight times that
...
Shealy~et~al.~\cite{Shealy2010} conducted an experimental study attempting to test the
...
modeling of jumper flight used in Hubbard~\cite{Hubbard2009} and
McNeil~et~al.~\cite{McNeil2012}.
...
catastrophic injuries, the raw data on which Shealy~et~al.~\cite{Shealy2015} was based is not
publicly available, thus making these results unverifiable. The data was
collected from press releases produced by the NSAA~\cite{Shealy2015}, which
...
loads~\cite{Scher2015}, even at low fall heights. They report on effects of
EFH but only test heights from \SIrange{0.23}{1.52}{\meter},  similar to
limitations in Shealy~et~al.~\cite{Shealy2010} by restricting ranges of their independent
...
argued~\cite{NSAA2008} that, because of rider and snow variability, terrain
...
of experiences for the users''~\cite{NSAA2008}, none of these in fact preclude
EFH analysis and rational engineering design.  This was shown clearly in
Hubbard and Swedberg~\cite{Hubbard2012} which examined quantitatively the effects of variations in
...
of their freestyle terrain park notebook~\cite{NSAA2015}, the jump landing
...
reference~\cite{NSAA2015} still contained the statement that ``Standards are
essentially impossible~\ldots''.
%
\begin{figure*}
  \centering
  %\includegraphics[width=6in]{figures/prinoth.png}
...
jumps~\cite{NSAA2015}. Nevertheless certifications and inspection standards
for bindings were developed, which led to fewer lower-extremity
equipment-related injuries~\cite{Bahniuk1996}.
...
inconvenience of shaping jumps using the methods in Levy~et~al.~\cite{Levy2015}.
...
%\begin{dci}
  MH served as a plaintiff's expert witness in the two case studies discussed
  above and in numerous other similar cases since.  CB testifies occasionally
  on behalf of plaintiffs in ski and snowboard injury cases. He has
  collaborated with Shealy and C.~D. Mote, Jr., Sher's doctoral advisor, on ski
  safety research, has participated in ASTM F27 since the 1980s on standards
  for bindings, boots, and skis, and holds patents on ski and snowboard binding
  designs intended to reduce injuries.
%\end{dci}
