...
Maximum allowed for parachuting by FAA: 3 m standard # AS8015

\todo[inline]{Each one of these cases tells a story of why EFH matters.}

\subsection{Charlene Vine v Bear Valley Ski Company}
%
In April of 2000, Ms. Charlene Vine's spine was broken on a jump constructed at
the Bear Valley Ski Resort. The jump shape was that of a ``tabletop'' which is
a common shape of ski and snowboarding jumps, but the jump's table, which is
typically flat and level, was concave. It is the jump constructor's intent that
jumpers fully clear the table when jumping, but Ms.~Vine landed short of the
end of the table. At the 11 meter landing distance the surface was sloped
upwards approximately 5 degrees. Figure~\ref{fig:vine-v-bear-valley} shows a
reasonable approximation of the jump at Bear Valley Ski Resort based on
measurements taken of the jump for the trial. The shape of this surface
highlights the detrimental effect of misaligning the surface tangent with the
jumper's flight path.
...
The green jump profile in the upper panel shows a possible jump design of
...
- on May 6, 2002 jury returned verdict with approx. half fault to each party

Vine landed 11 meters (36 feet) from takeooff (x coordinate).
