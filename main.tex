...
\begin{abstract}
  Terrain parks now exist in most ski resorts and for decades the
  epidemiological evidence has mounted showing that increases in injuries are
  correlated with their growing presence. We show through several case studies
  that large equivalent fall heights played significant roles in traumatic
  injuries at terrain parks. Even with this evidence, little has been done to
  make jumps safer by design. To help remedy this, we provide an accessible
  online tool for terrain park builders that can both evaluate existing jumps
  for impact danger and design jumps that control for impact danger.
\end{abstract}
