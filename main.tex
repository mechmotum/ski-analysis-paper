...
\subsection{Case 1: Vine v. Bear Valley Ski Company}
%
In April 2000, Ms.~Vine's lower spine was injured when she fell on a jump at
Bear Valley Ski Resort in California. The jump shape was a common form of
terrain park jump (Fig.~\ref{fig:vine-v-bear-valley}) called a ``table-top''.
...
the takeoff point~\todo{Double check this landing distance. Note that I wrote
12 meters below, which is different than here -- JKM}, the surface was sloped
...
\begin{figure}
  ...
  \caption{\textbf{Bear Valley Ski Resort jump compared to possible safer design}
  Top panel: Measured landing surface (solid black) and jumper flight paths
  (intermittent black) from 30\si{\degree} takeoff angle. The
  14~\si{\meter\per\second} takeoff speed is used as the design speed for a
  comparison jump (solid green) designed to have constant equivalent fall
  height of 1~\si{\meter}.
  Bottom panel: Equivalent fall height for both jumps in corresponding
...
execute the jump with less than 1~\si{\meter\per\second} precision in take-off
...
spinal injury, due to the likely lower impact forces when landing in the
rotated position.
...
injury is almost certain from falls this high, especially so if landing in a
...
\begin{figure}
  ...
  Top panel: Measured landing surface (solid black) and jumper flight paths
  (intermittent black) for 25~\si{\degree} takeoff angle. The
  16~\si{\meter\per\second} takeoff speed is used as the design speed for a
  comparison jump (solid green) which has constant equivalent fall height of
  1~\si{\meter}.
  Bottom panel: Equivalent fall height for both jumps in corresponding colors
...
park jumps motivates this work. The prior case studies show that no or poor
design can cause devastating consequences to snow sport participants.
Engineering analysis and design, based on the laws of mechanics, can be used
shape jump landings to limit equivalent fall heights. These laws go back
centuries to Issac Newton and Émilie du Châtelet~\cite{Zinsser2007}, are
introduced as early as high school physics, and are fundamental to engineering
education. Our motivation is consistent with the first canon of engineering
